...
> **What's a cluster?** A cluster is a bucket of similar requests that share one routing policy — its own cheap model, expensive model, threshold `τ`, and shadow rate. Cascadia assigns each incoming request to one of N clusters by a deterministic function of the prompt, so the same *kind* of traffic lands together and gets its threshold tuned on its own terms: easy, cheap-friendly prompts (short lookups, FAQs) can ride the small model, while harder ones (multi-step reasoning) escalate more often. One global threshold would overspend on the easy slice and underserve the hard one — separating traffic into clusters is what lets the feedback loop find the right cost/quality point for each slice independently. You can see them on the dashboard's `/clusters` page, each with its live threshold and escalation rate.
...
**Provider breadth.** Cascadia natively speaks 4 providers (OpenAI / Anthropic / Groq / xAI) and adds any number more through a **dynamic provider registry** — name them in `CASCADIA_PROVIDERS` and each gets its own base URL + key. Any OpenAI-shape host (HuggingFace, Together, Fireworks, vLLM, your endpoint) or Anthropic-shape host is a config entry, no code change — and mixed-provider cascades (e.g. `hf/…` cheap → `openai/…` expensive) now run in one proxy. See [docs/adding-a-provider.md](docs/adding-a-provider.md). For a genuinely different wire format (Bedrock, Vertex, the LiteLLM 100+), **stack Cascadia on top of LiteLLM**: Cascadia decides the cascade, LiteLLM handles the N-provider plumbing. They're orthogonal and compose — Cascadia hands LiteLLM a resolved `(provider, model, request)`, LiteLLM handles auth/retries/quirks. (Cascadia has no retries or fallback chains by design — that's LiteLLM's lane.)
