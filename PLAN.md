...
| Judge worker       | `services/judge-worker/`        | Python   | Consumes shadow-routed event pairs; runs judge prompts; writes scores. Judge ensemble (model + prompt variety) to fight bias. Built on the SOLID Agent Swarms pattern. |
...
This removes the **doc**, not the **pattern** — the architecture (substitution surfaces, composition-root wiring, no provider SDK imports outside the adapter layer) stands and remains the convention. Pointers to the file (`.dockerignore`, service READMEs, the dead pointer in the 2026-05-19 entry) were cleaned up; surviving "SOLID Agent Swarms pattern" / "SOLID-D" mentions describe the design, not the deleted file.
...
- `metrics.py` — Kendall's τ-b implemented by hand (no scipy dep — keeps judge-worker lean), agreement rate, position-bias mean, per-label distribution. Tested against perfect-concordance / perfect-discordance / zero-variance edge cases.
