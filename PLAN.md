...
- **Launcher.** `cli/` is dependency-free (Node 18+ built-ins): `demo`, `up`/`start` (key wizard, keys passed in-memory not to disk), `down`, `logs`, `doctor`. It locates the source via `$CASCADIA_HOME` → an enclosing checkout → a shallow `git clone` to `~/.cascadia/checkout` (`$CASCADIA_REPO`). The demo dashboard sets `CASCADIA_AUTH_DISABLED=true` for a frictionless first look; `up` keeps real auth on.
